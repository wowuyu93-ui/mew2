...
const WeChatApp: React.FC<WeChatAppProps> = ({ settings, onUpdateSettings, characters, onUpdateCharacters, onClose }) => {
  ...
  const activeChatIdRef = useRef<string | null>(null); // Ref to track active chat without staleness
  
  // Notification State
  const [notification, setNotification] = useState<NotificationState | null>(null);

  // Global Generation Lock (to prevent user from sending while ANY chat is generating)
  ...
  // Handle adding messages centrally to support notifications and prevent race conditions
  // CRITICAL FIX: Use functional update (prev => ...) to ensure we never use stale state
  const handleAddMessage = (charId: string, message: Message) => {
      ...
      // We use the REF here to check the *current* active chat, not the one captured when the function was created
      if (activeChatIdRef.current !== charId && message.role === 'model' && !message.isHidden) {
          // Find char info from current props (might be slightly stale but okay for notification content)
          // We can use a trick to find char from the functional update, but searching 'characters' prop is safe enough for metadata
          ...
          if (char) {
              ...
              // Auto hide after 3 seconds (User Request)
              setTimeout(() => {
  ...
  const handleCreateChar = () => {
    ...
    const char: Character = {
      ...
      contextMemory: '',
      historyCount: 20, // Default history count
      furnaceConfig: {
        autoEnabled: false,
        autoThreshold: 20,
        autoScope: 30,
      ...
      offlineConfig: {
        ...
        wordCount: 150,
        bgUrl: '',
        indicatorColor: '#f59e0b' // Default Amber
      },
      scenarios: [], // Initialize scenarios
      memories: [],
      messages: [],
      diaries: [],
      unread: 0,
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
          if (reader.result) {
            setNewChar(prev => ({ ...prev, avatar: reader.result as string }));
          }
      };
  ...
  const handleGlobalAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
          if (reader.result) {
            setTempGlobalPersona(prev => ({ ...prev, avatar: reader.result as string }));
          }
      };
      reader.readAsDataURL(file);
    }
  ...
  const handleDeleteChar = () => {
      if (!contextMenuCharId) return;
      if (window.confirm('确定要删除该联系人及其所有数据吗？此操作不可恢复。')) {
          // Check if the deleted character is the currently active chat
          if (activeChatId === contextMenuCharId) {
           ...
           <div className="space-y-4 bg-white p-4 rounded-xl shadow-sm">
             ...
             <div className="flex flex-col items-center mb-4">
                <div className="relative w-24 h-24 rounded-xl overflow-hidden bg-gray-200 border-2 border-dashed border-gray-400 mb-2 group">
                    {newChar.avatar ? (
                        <img src={newChar.avatar} className="w-full h-full object-cover" />
                    ) : (
             ...
             <div>
               <label className="block text-xs font-bold text-gray-700 mb-1">角色真名 (AI读取)</label>
               <input className="w-full p-2 border rounded bg-gray-50 focus:bg-white transition" placeholder="例如: 诸葛亮" value={newChar.name || ''} onChange={e => setNewChar({...newChar, name: e.target.value})} />
             </div>

             <div>
               <label className="block text-xs font-bold text-gray-700 mb-1">备注名 (列表显示)</label>
               <input className="w-full p-2 border rounded bg-gray-50 focus:bg-white transition" placeholder="例如: 丞相" value={newChar.remark || ''} onChange={e => setNewChar({...newChar, remark: e.target.value})} />
             </div>

             <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">性格/人设描述</label>
                <textarea className="w-full p-2 border rounded bg-gray-50 focus:bg-white h-24 text-sm" placeholder="描述角色的性格、说话方式、背景..." value={newChar.personality || ''} onChange={e => setNewChar({...newChar, personality: e.target.value})} />
             </div>
             
             <div>
               <label className="block text-xs font-bold text-gray-700 mb-1">System Prompt (系统指令 - 可自定义)</label>
               <textarea className="w-full p-2 border rounded bg-gray-900 text-green-400 h-40 text-[10px] font-mono leading-relaxed" value={newChar.systemPrompt || DEFAULT_SYSTEM_PROMPT} onChange={e => setNewChar({...newChar, systemPrompt: e.target.value})} />
             </div>
      ...
      return (
        <div className="divide-y divide-gray-200">
          {sortedChars.map(char => {
             // Filter out theater/offline/hidden messages from preview to keep main chat clean
             const lastMsg = [...char.messages]
                .filter(m => m.mode !== 'theater' && m.mode !== 'offline' && !m.isHidden)
                .pop();
                 ...
                 <div className="flex-1 min-w-0 pointer-events-none">
                   <div className="flex justify-between items-baseline mb-1">
                     <h3 className="font-medium text-gray-900 truncate flex items-center gap-1">
                         {char.remark}
                         {char.isPinned && <i className="fas fa-thumbtack text-xs text-gray-400 rotate-45"></i>}
                     </h3>
        ...
        return (
            <div className="p-4 bg-gray-50 h-full overflow-y-auto">
                <div className="bg-white p-6 rounded-xl shadow-sm space-y-6">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">我的名字</label>
                        <input 
                            value={tempGlobalPersona.name}
                            onChange={(e) => setTempGlobalPersona({...tempGlobalPersona, name: e.target.value})}
                            className="w-full p-3 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:border-[#07c160] transition"
                        />
                    </div>

                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">我的性格/简介</label>
                        <textarea 
                            value={tempGlobalPersona.description}
                            onChange={(e) => setTempGlobalPersona({...tempGlobalPersona, description: e.target.value})}
                            className="w-full p-3 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:border-[#07c160] transition h-32 resize-none"
                            placeholder="AI 将根据这个描述来认识你..."
                        />
                    </div>

                    <button 
                        onClick={saveGlobalPersona}
                        className="w-full py-3 bg-[#07c160] hover:bg-[#06ad56] text-white font-bold rounded-xl shadow-lg shadow-green-200 active:scale-95 transition-all"
                    >
                        保存全局设置
                    </button>
                    
                    <p className="text-xs text-gray-400 text-center px-4">
                        * 这是你的默认身份。你可以在每个角色的聊天设置中单独覆盖此身份。
                    </p>
                </div>
            </div>
        )
    ...
    <div className="h-full bg-gray-100 flex flex-col text-black relative">
      ...
      <div className="bg-[#ededed] p-3 border-b border-gray-300 flex justify-between items-end pb-2 sticky top-0 z-10">
         ...
         <div className="flex gap-4 mr-2">
            ...
            <i className="fas fa-plus-circle text-gray-900" onClick={() => {setActiveTab(WeChatTab.CONTACTS); setIsCreating(true)}}></i>
         </div>
      </div>
      ...
      <div className="bg-[#f7f7f7] border-t border-gray-300 flex justify-around py-2 pb-6 sm:pb-2">
         {[
           {id: WeChatTab.CHATS, icon: 'comment', label: '微信'},
           {id: WeChatTab.CONTACTS, icon: 'address-book', label: '通讯录'},
           {id: WeChatTab.MOMENTS, icon: 'compass', label: '发现'},
           {id: WeChatTab.ME, icon: 'user', label: '我'},
         ].map(tab => (
           <button 
             key={tab.id}
             onClick={() => {
                setActiveTab(tab.id as WeChatTab);
                setIsCreating(false);
             }}
             className={`flex flex-col items-center gap-0.5 ${activeTab === tab.id ? 'text-[#07c160]' : 'text-gray-900'}`}
           >
      ...
      {/* Notification Overlay (Main App Level) */}
      <NotificationBubble />
      ...
      {contextMenuCharId && (
        <div className="absolute inset-0 bg-black/20 z-50 flex flex-col justify-end" onClick={() => setContextMenuCharId(null)}>
            <div className="bg-white rounded-t-2xl p-4 animate-slide-up space-y-2 shadow-2xl pb-8" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={handleTogglePin} className="w-full py-3 bg-gray-100 rounded-xl font-bold text-gray-800 flex items-center justify-center gap-2">
                    <i className="fas fa-thumbtack"></i>
                    {characters.find(c => c.id === contextMenuCharId)?.isPinned ? '取消置顶' : '置顶聊天'}
                </button>
                <button onClick={handleDeleteChar} className="w-full py-3 bg-red-50 rounded-xl font-bold text-red-600 flex items-center justify-center gap-2">
